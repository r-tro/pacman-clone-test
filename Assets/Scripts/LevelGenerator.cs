    ...
    public void LevelMap()
    {
        for (int i = 0; i < levelMap.GetLength(0); i++) // rows (y)
        {
            for (int j = 0; j < levelMap.GetLength(1); j++) // columns (x)
            {
                ...
                //Debug.Log("topr left " + tileTopLeft + "levelMap[" + i + "," + j + "]");
                ...
                tileTopRight.transform.position = new Vector3(     (StartPoint.x + (((levelMap.GetLength(1) * 2) - 1) * TileSize)) - (TileSize * j)    , StartPoint.y - (TileSize * i), 0); // x = (((14 tiles across * 2) - 1) * TileSize))
                ...
                if (i > 0 && j < levelMap.GetLength(1)) // check piece rotation if next to certain pieces
                {
                    if (levelMap[i, j] == 1) // OUTSIDE CORNERS
                    {
                        ...
                        if (j + 1 <= levelMap.GetLength(1) && (levelMap[i + 1, j] == 2 && j > 0)) // Top Right (OUTSIDE) cCorner
                        {
                            tileTopLeft.transform.Rotate(0f, 0f, -90f);
                            tileTopRight.transform.Rotate(0f, 0f, -90f);
                        }
                ...
                {
                    if (levelMap[i, j] == 1 && (i == 0 && j == 0)) // levelMap[0,0];
                    {
                        Debug.Log("topr right " + tileTopRight + "levelMap[" + i + "," + j + "]");
                        tileTopLeft.transform.Rotate(0f, 0f, 0f);
                        tileTopRight.transform.Rotate(0f, 0f, -90f);
